...
export function AuthProvider(props) {
  ...
  // Initialize state based on server/client environment
  const [state, dispatch] = useReducer(reducer, {
    ...initialState,
    // On server side, immediately mark as initialized to allow page rendering
        ...
        try {
          if (getCookie(process.env.NEXT_PUBLIC_AUTHENTICATION_ID_TOKEN)) {
            const user = await authApi.me(
              getCookie(process.env.NEXT_PUBLIC_AUTHENTICATION_ID_TOKEN)
            );
  ...
  const confirmSignup = async (email, code, password, confirmPassword) => {
    ...
    try {
      response = await axios.post(
        ...
        },
        {
          headers: {
            ...
            "Cocina-Digital-Language":
              getCookie(process.env.NEXT_PUBLIC_LANGUAGE_COOKIE_NAME) || "en",
          },
        }
      );
      ...
      if (typeof window !== "undefined") {
        ...
        if (
          response?.data?.userProfile?.activeProfileImage !== undefined &&
          response?.data?.userProfile?.activeProfileImage?.images?.length >= 2
        ) {
          // Note: userProfile.activeProfileImage.images[1] is the 100x100 image:
          // save userProfilePicture to sessionStorage:
          globalThis.sessionStorage.setItem(
            "userProfilePicture",
            response?.data?.userProfile?.activeProfileImage?.images[1]?.src
          );
        }
      ...
      const expires = new Date(
        Date.now() +
          process.env.NEXT_PUBLIC_SESSION_TIMEOUT_IN_MINUTES * 60 * 1000
      );
      setCookie(
        ...
        if (response?.data?.subscriptions?.length) {
          ...
          for (const subscription of arrSubscriptions) {
            ...
            if (
              ...
              subscription.endOfBillingPeriod !== undefined
            ) {
              ...
              if (currentDateTime <= endOfBillingPeriod) {
                ...
                setCookie(
                  process.env
                    .NEXT_PUBLIC_SUBSCRIPTION_EXPIRATION_BILLING_DATE_COOKIE_NAME,
                  subscription?.endOfBillingPeriod,
                  {
                    secure: true,
                    path: "/",
    ...
    try {
      ...
      if (code) {
        response = await axios
          .post(
            process.env
              .NEXT_PUBLIC_AUTHENTICATION_CONFIRM_FORGOT_PASSWORD_ENDPOINT,
            {
              username: email,
              newPassword: password,
              confirmationCode: code,
            },
            {
              headers: {
                "Content-Type": "application/json",
                "Cocina-Digital-Language":
                  getCookie(process.env.NEXT_PUBLIC_LANGUAGE_COOKIE_NAME) ||
                  "en",
              },
            }
          )
